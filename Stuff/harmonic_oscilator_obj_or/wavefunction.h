...
class Wavefunction{
    ...
    protected:
        ...
        virtual double evaluateSecondDerivative(); // Evaluates the second derivative if an analytical form exists
        virtual double numericalSecondDerivative(); // Evaluates the secod derivative numerically from the analytical wf
        ...
        should be passed one by one if we pass the entire system. By passing the pointer we pass one single variable!!, the memory address */
        class System* s; 
