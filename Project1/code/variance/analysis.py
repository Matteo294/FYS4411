...
class dataAnalysisClass:
    ...
    def autocorrelation(self):
        self.func_name = "autocorrelation"
        self.acf = np.zeros(int(len(self.data)/2))
        for k in range(0, int(len(self.data)/2)):
            self.acf[k] = np.corrcoef(np.array([self.data[0:len(self.data)-k], self.data[k:len(self.data)]]))[0,1]

    # Bootstrap
    @timeFunction
    def bootstrap(self, nBoots = 1000):
        self.func_name = "bootstrap"
    ...
    def plotAutocorrelation(self):
        font = {'fontname':'serif'}
        plt.plot(range(1, int(len(self.data)/2)), self.acf[1:], 'r-')
        plt.ylim(-1, 1)
        plt.xlim(0, int(len(self.data)/2))
